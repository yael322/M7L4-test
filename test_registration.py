...
def test_authenticate_success(setup_database):
    ...
    assert result is True, "Пользователь должен успешно авторизоваться."


#Тест несуществующего пользователя
def test_authenticate_nonexistent_user(setup_database):
    result = authenticate_user('nouser', 'nopassword')

    assert result is False, "Несуществующий пользователь не должен проходить авторизацию."
